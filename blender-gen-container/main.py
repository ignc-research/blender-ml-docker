...
def scene_cfg(camera, i):
    ...
    while (repeat):
        ...
        if cfg.NumberOfObjects == 1:
            ...
            mode = 'ab'
        if(not repeat):
            fname = cfg.output_path + "/object/labels/{:06}.txt".format(i)
            f = open(fname, mode)
...
def render(camera, depth_file_output, test_flag):
    ...
    for i in range(cfg.numberOfRenders):
        try:
            print('--------------------in_loop-----------------')
            r = requests.post('http://127.0.0.1:3001/renderprogress', data={'current': i+1})
            print('success')
            time.sleep(2)
            ...
            time.sleep(2)
            pass
        bpy.context.scene.render.filepath = cfg.output_path + '/object/images/{:06}.png'.format(i)
...
def main():
    ...
    if args.test:
        import cv2
        image = cv2.imread(cfg.output_path + 'object/images/{:06}.png'.format(cfg.numberOfRenders-1))

        # load data
        BB = np.loadtxt("model_bounding_box.txt")
        label = np.loadtxt(cfg.output_path + "/object/labels/{:06}.txt".format(cfg.numberOfRenders-1))
